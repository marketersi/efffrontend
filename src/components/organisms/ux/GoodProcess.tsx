  ...
  useEffect(() => {
    ...
    const checkSvgLoaded = setInterval(() => {
      if (path.getTotalLength() > 0) {
        ...
        //   console.log(circles.length , 'console.log(circles.length);');
      ...
      setTimeout(() => {
        const circles = document.querySelectorAll(".circle");
        console.log("Total circles found:", circles.length); // Debugging ke liye
      
        if (circles.length >= 3) { // Ensure 3 circles mil rahe hain
          const tl = gsap.timeline({
            scrollTrigger: {
              trigger: sectionRef.current,
              start: "top 60%",
          ...
          tl.fromTo(
            ...
            { scale: 0.3, opacity: 0 },
            {
              ...
              opacity: 1,
              duration: 1.2,
              stagger: 0.3, // Thoda stagger badhakar try karein
              ease: "power1.in",
            }
          );
        }
